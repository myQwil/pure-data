...
#include "m_private_utils.h"
    ...
    /* same as `stpcpy`, but local and under a different name */
static char *chaincpy(char *dest, const char *src)
{
    while ((*dest++ = *src++) != '\0')
        /* nothing */;
    return --dest;
            ...
            case A_DOLLSLICE:
            {
                ...
                *at = slice->s_atom;
                ...
                else if (*slice->s_sep->s_name != ' ')
                {
                    ...
                    int next = 0;
                    buf[0] = '\0', bp = buf;
                    if (pre) bp = chaincpy(bp, pre->s_name);
                    for (; size--; i += step, bp += strlen(bp), next = 1)
                    {
                        if (next) bp = chaincpy(bp, sep->s_name);
